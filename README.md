...
ゲームを起動して究極錬成画面を開いた状態で実行します。

> **重要**: プリンセスコネクト Re:Dive は管理者権限で動作しているため、このツールも**管理者権限で起動した cmd または PowerShell** から実行する必要があります。非管理者で起動するとマウス操作が拒否されます（UIPI 制限）。

```
uv run python main.py
```

デバッグモード（ROI オーバーレイ表示）:
