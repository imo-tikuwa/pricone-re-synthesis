...
const steps = [
  ...
  {
    text: 'ultimate-synthesis.exe を実行する（UAC の確認が表示されたら「はい」を選択）',
    note: 'ゲームが起動していない場合はエラーになります',
  },
...
<template>
  ...
  <Transition name="loader">
    <div v-if="!ready" class="fixed inset-0 z-[9999] bg-surface-100 dark:bg-surface-900">
      <ProgressBar
        ...
        :pt="{
          root: {
            style: 'height:3px;border-radius:0;border:none;background:var(--p-surface-200);',
          },
        }"
      />
    </div>
  ...
  <Transition name="page">
    <div v-if="ready">
      <CookieConsentBanner />
      <div
        class="min-h-screen bg-surface-100 text-surface-900 dark:bg-surface-900 dark:text-surface-0"
      >
        <!-- ヘッダー -->
        <header
        ...
        <main class="mx-auto max-w-[960px] space-y-16 px-4 py-12">
          ...
          <Message severity="warn" :closable="false" icon="pi pi-exclamation-triangle">
            <div class="space-y-1">
              <p class="font-bold">本ツールは開発・メンテナンスを終了しました</p>
              <p class="text-sm">
                ゲーム本体に「カスタム錬成」機能が実装され、本ツールが担っていた役割がゲーム内でカバーされるようになったため、2026
                年 7 月 19
                日をもって開発を終了しました。下記の最新版が最終リリースです。今後の更新・不具合対応・サポートは行いません。
              </p>
            </div>
          </Message>
          ...
          <section class="space-y-4">
            <h2
              class="border-l-4 border-surface-400 pl-3 text-2xl font-bold dark:border-surface-500"
            >
              動作要件
            </h2>
                  ...
                  <tbody>
                    <template v-for="item in uiItems" :key="item.no">
                      <tr class="border-b border-surface-200 dark:border-surface-700">
                        <td class="py-2.5 pr-4 text-base font-bold text-surface-500">
                          {{ item.no }}
                        </td>
                        <td class="whitespace-nowrap py-2.5 pr-4 font-medium">{{ item.name }}</td>
                        <td class="py-2.5 text-surface-600 dark:text-surface-400">
                          {{ item.desc }}
                        </td>
                      </tr>
                      <tr
